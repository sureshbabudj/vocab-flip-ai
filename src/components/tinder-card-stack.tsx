...
const TinderCard: React.FC<TinderCardProps> = ({ card, onSwipe, isActive }) => {
  ...
  return (
    <div
      {...handlers}
      className={`absolute w-full h-full transition-all duration-300 touch-none ${
        isActive ? 'z-10' : 'z-0'
      }`}
...
export const TinderCardStack: React.FC = () => {
  ...
  const handleSwipe = useCallback(
    (direction: 'left' | 'right') => {
      ...
      const difficulty = direction === 'right' ? 'easy' : 'hard';
  ...
  if (currentIndex >= filteredCards.length) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100dvh-200px)] text-center space-y-6 p-6">
        ...
        <p className="text-muted-foreground max-w-md">
          You've swiped through all {filteredCards.length} cards.
        </p>
    ...
    <div className="flex flex-col h-[calc(100dvh-200px)]">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="relative w-full max-w-md h-full">
          {/* Show next few cards as background stack */}
          {filteredCards
